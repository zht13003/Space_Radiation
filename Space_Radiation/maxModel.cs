/********************************************************************
	created:	2020/12/03
	created:	3:12:2020   13:02
	filename: 	maxModel.cs
	file path:	Space_Radiation
	file base:	maxModel
	file ext:	cs
	author:		Kaguya
	
	purpose:	通过getFlux函数，输入高度（km）、地磁纬度，输出粒子能谱：
                4*510的矩阵，第一行为500项递增的电子能量（MeV），第二行
                为对应的电子通量（cm^-2*s^-1），第三行为质子能量，第四行
                为对应的质子通量。
                maxModel为太阳极大年模型，minModel为太阳极小年模型。
                源语言为C，由作者改写为C#，故用到数组指针。
*********************************************************************/
using System;
